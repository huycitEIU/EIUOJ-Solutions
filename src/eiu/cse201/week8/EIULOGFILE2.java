...
public class EIULOGFILE2 {
    public static void main(String[] args) {
        ...
        while (m-- > 0) {
            long event = io.nextLong();

            int index = binarySearch(event, logs);

            if (index == -1) {
                io.printInt(-1);
            } else {
                io.printLong(logs[index]);
            }
            io.printChar(' ');
        }
    ...
    static int binarySearch(long event, long[] logs) {
        int l = 0;
        int r = logs.length - 1;
        int index = -1;

        if (event > logs[r]) return index;
